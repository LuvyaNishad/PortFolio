...
export default function EtherealShadow({
  ...
  const displacementScale = animation ? mapRange(animation.scale, 1, 100, 20, 90) : 0;
  const animationDuration = animation ? mapRange(animation.speed, 1, 100, 1400, 70) : 1;
            ...
            <defs>
              <filter id={id}>
                <feTurbulence
                  result="undulation"
                  numOctaves="2"
                  baseFrequency={`${mapRange(animation.scale, 0, 100, 0.001, 0.0006)},${mapRange(
                    animation.scale,
                    0,
                    100,
                    0.004,
